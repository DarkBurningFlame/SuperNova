...
#if DEV_BUILD_NOVA
            LevelPermission perm = Group.ParsePermOrName(args[2], LevelPermission.Nova);
            if (perm == LevelPermission.Nova) return;
#else
...
#if DEV_BUILD_NOVA
            Command.Find("SetRank").Use(Player.Nova, p.name + " " + rank.Name);
#else
