...
namespace SuperNova.Generator 
{
    ...
    public sealed class MapGen 
    {
        ...
        internal static void SetRealmPerms(Player p, Level lvl)
        {
            ...
            const LevelPermission rank = LevelPermission.Nova;
            lvl.BuildAccess.Whitelist(Player.Nova, rank, lvl, p.name);
            lvl.VisitAccess.Whitelist(Player.Nova, rank, lvl, p.name);

            Group grp = Group.Find(Server.Config.OSPerbuildDefault);
            if (grp == null) return;

            lvl.BuildAccess.SetMin(Player.Nova, rank, lvl, grp);
        }
    }
}
#else
