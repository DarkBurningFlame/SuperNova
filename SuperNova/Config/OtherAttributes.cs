...
namespace SuperNova.Config {
    ...
    public sealed class ConfigPermAttribute : ConfigAttribute {
        ...
        public override object Parse(string raw) {
#if DEV_BUILD_NOVA
            LevelPermission perm = Group.ParsePermOrName(raw, LevelPermission.Nova);
            if (perm > LevelPermission.Nova)
            {
            ...
            if (perm > LevelPermission.Nova)
            {
                Logger.Log(LogType.Warning, "Config key \"{0}\" cannot be above Nova.", Name);
                perm = LevelPermission.Nova;
            }
            return perm;
        }
#else
