        ...
        static bool WarnPillaring(Player p, ZSData data, ushort x, ushort y, ushort z, bool nonReplacable) {
            if ((!nonReplacable && data.BlocksStacked == 2) || (nonReplacable && data.BlocksStacked == 1)) {
                ...
                if (!data.PillarFined) {
                    Chat.MessageFromOps(p, "  &cWarning: λNICK &Sis pillaring!");
#if DEV_BUILD_NOVA
                    Command.Find("Take").Use(Player.Nova, p.name + " 10 Auto fine for pillaring");
#else
...
#if DEV_BUILD_NOVA
                    ModAction action = new ModAction(p.name, Player.Nova, ModActionType.Kicked, "Auto kick for pillaring");
#else
